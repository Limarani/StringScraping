        ...
        public string FTP_Riverside(string address, string parcelNumber, string ownername, string searchType, string orderNumber, string directParcel)
        {
            ...
            using (driver = new PhantomJSDriver())
            {
                if (searchType == "titleflex")
                {
                    ...
                    {
                        HttpContext.Current.Session["Zero_RiverSide"] = "Zero";
                ...
                try
                {
                    driver.FindElement(By.XPath("//*[@id='ngb-tab-0-panel']/span/tr-search-options/div/div/div/div[1]/div/div/div/tr-masked-text-input/input")).SendKeys(parcelNumber);
                    driver.FindElement(By.XPath("//*[@id='ngb-tab-0-panel']/span/tr-search-options/div/div/div/div[2]/div/button")).Click();
                    Thread.Sleep(8000);
                    ...
                    foreach (string View in viewbilllist)
                    {
                        ...
                        try
                        {
                            ...
                            for (int s = 1; s <= Instalmentcount.Count; s++)
                            {
                                ...
                                string InstalmentTh = driver.FindElement(By.XPath("//*[@id='tblInstallment']/tbody/tr[" + s + "]/th")).Text;
                                string[] instalarray = InstalmentTh.Split('|');
                                string Payyear = GlobalClass.After(instalarray[0], "Payable Year:");
                                string Paybill = GlobalClass.After(instalarray[1], "Bill Number");
                                string Paytype = instalarray[2];
                                gc.insert_date(orderNumber, Parcel_number, 1750, exemptionresult, 1, DateTime.Now);
                                IWebElement InstallmentTable = driver.FindElement(By.XPath("//*[@id='lxT423']/table/tbody"));
                                IList<IWebElement> Installmentrow = InstallmentTable.FindElements(By.TagName("tr"));
                                IList<IWebElement> Installmentid;
                                ...
                                foreach (IWebElement Installment in Installmentrow)
                                {
                                    ...
                                    if (Installmentid.Count != 0)
                                    {
                                        Installmentresult = Installmentresult1 + "~" + Installmentid[0].Text + "~" + Installmentid[1].Text + "~" + Installmentid[2].Text + "~" + Installmentid[3].Text;
